...
impl Cpu {
    ...
    pub fn run_commands(&mut self, bus: &mut Bus){
        let hi = bus.ram_read_byte(self.pc) as u16;
        let lo = bus.ram_read_byte(self.pc + 1) as u16;
        let command: u16 = (hi << 8) | lo;

        // println!("{}, {}", hi, lo);

        let nnn = command & 0x0FFF;
        let nn = (command & 0x0FF) as u8;
        let n = (command & 0x00F) as u8;
        let x = ((command & 0x0F00) >> 8) as u8;
        let y = ((command & 0x00F0) >> 4) as u8;

        match (command & 0xF000) >> 12 {
            0x0 => {
                match nn {
                    0xE0 => {
                        bus.clear_screen();
            ...
            0x8 => {
                ...
                match n {
                    ...
                    _ => panic!(
                        "Unrecognized 0x8XY* command {:#X}:{:#X}",
                        self.pc,
                        command
                    ),
                };
            ...
            0xC => {
                ...
                let number = rand::thread_rng().sample(interval);
            ...
            0xE => {
                match nn {
                    0xA1 => {
                        // if(key()!=Vx) then skip the next command
            ...
            _ => panic!("Unrecognized command {:#X}:{:#X}", self.pc, command),
        }
    }
